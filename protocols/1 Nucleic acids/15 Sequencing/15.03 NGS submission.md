...
6. Dilute a small amount of the pooled sample to 1 ng/uL and **run Bioanalyzer** ([protocol](../12%20Gels/12.03%20Bioanalyzer.md)).
...
- **Read length:** Should cover you sample. If paired-end, the reads can be merged if there is at least 10 bp overlap (20 bp recommended) between the two reads, so 2 x 100 bp covers sequences up to 180 bp. With little overlap it is hard to know how to correctly combine the two reads (you need to account for mismatches).
- **Number of reads:** We typically aim for 5M reads per sample.
### MiSeq
(Total reads that you receive; e.g., 100M 100 bp would give 100B single reads, but 100M 150 bp would give 200M paired-end reads.)
...
### NextSeq
(Total reads that you receive; e.g., P4 50 bp would give 1.8B single reads but 3.6B paired-end reads.)
